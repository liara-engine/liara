...
**Interface integrity**, in `liara-interfaces` only. `reusable-abi-header-portability.yml` compiles every public header standalone and warning-free as C and C++ across several language standards. `reusable-abi-interface-rules.yml` enforces the [naming](https://liara-engine.liara-engine-documentation.workers.dev/liara-interfaces/latest/guides/naming/) and [type](https://liara-engine.liara-engine-documentation.workers.dev/liara-interfaces/latest/guides/types/) rules of the interface guide. `reusable-abi-layout-freeze.yml` freezes struct layouts against a generated golden header. `reusable-abi-snapshot.yml` snapshots the ABI surface and diffs it against the pull request base to compute the required version bump. `reusable-abi-report.yml` aggregates the four into one sticky comment.
