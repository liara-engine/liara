---
title: "ADR 0010: Vulkan directly, without a rendering abstraction layer"
description: The third answer to the same question, why bgfx and wgpu were never weighed, and the rule that lets Vulkan-Hpp and VMA in anyway.
sidebar:
  label: "0010 · Vulkan, no abstraction"
  order: 10
---

| Status   | Date       | Deciders |
|----------|------------|----------|
| Proposed | 2026-08-26 | Antoine  |

*Proposed rather than Accepted: real Vulkan work starts in v0.1 and only a placeholder renderer exists today. It moves to Accepted when v0.1 closes.*

## Context

This is the third engine project I have started, and the graphics API question has been answered three times.

The first attempt, years ago, was on Windows, and I chose DirectX, partly because I was following Arash Khatami's Game Engine Series, mostly because on Windows it was the obvious choice. Cross-platform support was not a consideration. That project never got past a sketch.
...
One thing carried across all three attempts: I have little patience for backwards compatibility and legacy support, which I find usually costs more than it returns. It is why the engine targets Vulkan 1.3 and recent GPUs, and why I develop on Arch rather than Debian.
...
**A rendering abstraction layer**, bgfx, sokol_gfx or wgpu, with Vulkan as one backend among several. I should be straightforward here: these were not weighed at the time. I did not know they existed when the choice was made, and I only learned about them while writing this record.

Knowing about them now does not change the answer, and the reason is worth stating plainly instead of being reconstructed as an evaluation I never performed. I build things to understand them from the bottom, and an abstraction layer removes precisely the layer I want to be working in. It would let me draw a triangle in a fraction of the time and teach me nothing about how a triangle gets drawn. Since learning graphics programming is the first of the three reasons this project exists, that is disqualifying regardless of the library's quality.
...
That is the rule I apply, and it is narrower than "no abstractions": I allow a library that does better something I have already done myself and still understand. Vulkan-Hpp is the same API with C++ types over a model I know. VMA solves an allocation problem I have solved worse. The same test is why I use the standard library without hesitation, since over the years I have reimplemented a fair amount of what it does, sometimes for coursework and sometimes because I did not know it was already there, and it is why bgfx fails: it would be the first time I used something whose model I had never built.
...
- Hardware support becomes an actual barrier to someone shipping a game with the engine, which is a v0.6 or later question and not a v0.1 one.
- A second backend becomes necessary for a platform worth supporting, at which point the abstraction question genuinely reopens and should be answered with a new record rather than by retrofitting one onto the interface.
