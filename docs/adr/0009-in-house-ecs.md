---
title: "ADR 0009: A hand-written ECS rather than an existing library"
description: Why EnTT was rejected for a reason that has nothing to do with performance, and what a hand-written sparse-set ECS costs.
sidebar:
  label: "0009 · Hand-written ECS"
  order: 9
---
...
*Proposed rather than Accepted: the ECS arrives in v0.2 and nothing has been built on it yet. This records the decision before the investment, which is the only point at which changing my mind is still cheap. It moves to Accepted when v0.2 closes.*
...
**EnTT.** Header-only, mature, extremely fast, and the default answer for this problem in C++. Rejected, and I want to be honest about the reason, because "it wouldn't have been fast enough" or "it wouldn't have fit the architecture" would both be false. I rejected it because adopting it would remove the part of this project I am here for.
...
The fuller version is that I work on things I can see the bottom of. Assembling a project out of libraries whose internals I do not know is, for me, a different activity from building something, one that happens to produce a similar artifact, and one I lose interest in quickly. On a project with no deadline and no client, sustained interest is the actual constraint, and a technically superior choice that I stop working on is worse than an adequate one I finish. This is a bad criterion for a team and a decisive one for me, so it is recorded here instead of being left implicit and rediscovered later as inconsistency.
...
It will be slower than EnTT, probably by a lot at first. That is accepted, with a caveat: the ECS is benchmarked in CI from v0.2 onward, so "slower" stays a number I am looking at rather than a thing I assume is fine.
...
The API is mine, so it can change to fit what the engine turns out to need instead of what a library author anticipated. The matching cost is that there is no upgrade path and nobody answering questions about it.
...
- The benchmarks show the ECS is the frame-time bottleneck and archetype storage is the fix, which is a change of storage strategy under the same API rather than a change of this decision.
- I find myself maintaining it instead of building the engine with it. Writing it is the point, and keeping it alive at the expense of everything else is not. If that happens, the honest move is to swap in EnTT and say so in a superseding record.
