---
title: "ADR 0008: Two-tier CI, and reusable workflows over composite actions"
description: Why a fast branch tier and a heavy integration tier answer two different questions, and why the pipelines live in the organization's .github repository.
sidebar:
  label: "0008 · Two-tier CI"
  order: 8
---
...
I designed the second one first: a single pipeline on pull requests and `main`, assembling the workspace and building the full matrix. Then I looked at what that means day to day and the answer is nothing at all, since a branch gets no feedback until a pull request exists. Which leaves two options, both bad. Open the pull request early just to see a build, and run the heavy matrix on work-in-progress, or work blind and hit a wall of failures at review time.

Sizing the single pipeline down instead does not work either, because then nothing ever checks composition, which is the property the multi-repository layout of [ADR 0001](../0001-multi-repository-layout/) exists to protect.
...
**Standalone builds in both tiers.** Simpler, faster, and it is what the branch tier does. Rejected for the integration tier, because a module built alone against an installed contract cannot notice it has broken a sibling, and inter-module breakage is the specific risk that comes with splitting a codebase across repositories. The workspace build is the only thing that answers that, and it costs a few minutes.
...
The branch tier is deliberately incomplete and that has to stay explicit. No Windows, no shared libraries, no clang-tidy, no composition check. A green branch build means "this compiles" and not "this is ready", and I have to keep reading it that way.
...
The sticky comment is the interface to a run, and its budget is finite, since nine legs of build logs would overflow GitHub's comment limit. A passing leg gets one line and only failures carry their output, because a report that sends me back to the raw logs is not worth reading.
...
- The branch tier stops being fast enough to run on every push, which calls for narrowing it rather than for widening the integration tier.
